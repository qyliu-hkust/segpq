...
class CompressedPGMIndex {
    ...
    std::vector<CompressedLevel> levels;  ///< The levels composing the compressed index.构建压缩索引的级别数量
...
public:
    ...
    CompressedPGMIndex(Iterator first, Iterator last) : n(std::distance(first, last)) {
        ...
        // Compress the slopes
        auto[tmp_table, map, intercepts] = merge_slopes(segments);  // 使用 merge_slopes 函数压缩斜率，通过合并相似斜率并计算相应的截距，减少了索引中独特斜率的数量，从而减少了存储需求。
        slopes_table = tmp_table;   // 存储的斜率

        // Build levels, 开始构建压缩索引，从第一层开始，自底向上构建压缩索引
        ...
        for (int i = EpsilonRecursive == 0 ? 1 : n_levels - 1; i > 0; --i) {
            auto l = levels_offsets[i - 1];     //当前级别的起始和结束偏移
            auto r = levels_offsets[i];
            auto prev_level_size = i == 1 ? n : l - levels_offsets[i - 2];      // 计算上一级的大小,若只有一级索引，则prev_level_size 将被赋值为 n（数据元素总数）
            levels.emplace_back(segments.begin() + l, segments.begin() + r,     // levels：CompressedLevel类型，为当前级别创建 CompressedLevel 实例：使用 emplace_back 直接在 levels 向量中构造一个新的 CompressedLevel 对象
    ...
    ApproxPos search(const K &key) const {
        ...
        if constexpr (EpsilonRecursive == 0) {
            ...
            auto pos = std::min<size_t>(level(slopes_table, i, k), level.get_intercept(i + 1));     // 从压缩索引中获取近似位置
...
struct CompressedPGMIndex<K, Epsilon, EpsilonRecursive, Floating>::CompressedLevel {        // 自定义一个数据类型，并使用双冒号指出其属于CompressedPGMIndex类
...
protected:
    ...
    struct SegmentData {
        ...
        inline size_t operator()(const K &origin, const K &k) const {   // 返回近似pos ￥￥￥
    ...
    sdsl::sd_vector<> ef;               ///< The Elias-Fano structure on the segment.   段的EF编码信息
...
public:
    ...
    ApproxPos search(const K &key) const {
        auto k = std::max(first_key, key);
        auto[r, origin] = pred(k - first_key);      // 使用 pred 方法在EF编码的 ef 结构中查找键值 key 减去 first_key 的排名。
        auto pos = std::min<size_t>(segments[r](origin + first_key, k), segments[r + 1].intercept);     // 根据排名和 segments 向量计算近似位置 pos。
        auto lo = PGM_SUB_EPS(pos, Epsilon);
        auto hi = PGM_ADD_EPS(pos, Epsilon, n);
        return {pos, lo, hi};
    }
...
private:
    // 用于在 Elias-Fano 编码结构 ef 中查找给定排名（rank）的元素的近似位置。
